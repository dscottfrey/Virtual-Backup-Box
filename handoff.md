...
15. **Per-KnownCard security-scoped bookmark (skip picker entirely)** — Today, even when a known card is mounted and we detect it, the user still has to tap through the system file picker (one extra tap on "Open"). The "fully tap-to-select" win is: store a security-scoped bookmark on each `KnownCard` at first pick, then on re-insertion resolve the bookmark and skip the picker entirely. Requires a data-model change (one new `bookmarkData: Data?` property on `KnownCard` + tiny SwiftData migration) and bookmark-resolution code in the "Choose Previous" path. Hold until the 2026-05-12 hybrid (mount-detect + tappable "Choose Previous" that opens the picker pre-navigated) is validated on device. The hybrid is intentionally low-risk because it never bypasses the iOS sandbox prompt; this follow-up does, so it deserves its own commit and device test.
